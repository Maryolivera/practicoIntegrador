...
async function cargarPaises() {
  try {
    ...
    listaPaises = datos;
    paisesConCapital = listaPaises.filter(
      (p) => p.capital && p.capital.length > 0
    );
    paisesConBandera = listaPaises.filter((p) => p.flags && p.flags.svg);
    paisesConFronteras = listaPaises.filter(
      (p) => p.borders && p.borders.length > 0
    );
  } catch (error) {
...
function preguntaCapital() {
  const pais =
    paisesConCapital[Math.floor(Math.random() * paisesConCapital.length)];
  ...
  document.getElementById(
    "progreso"
  ).textContent = `🟡 Pregunta ${numeroPregunta} de ${totalPreguntas}`;
  document.getElementById(
    "pregunta"
  ).textContent = `¿Cuál es la capital de ${nombrePais}?`;
...
function preguntaBandera() {
  const pais =
    paisesConBandera[Math.floor(Math.random() * paisesConBandera.length)];
  const nombrePais = pais.name.common;
  ...
  document.getElementById(
    "pregunta"
  ).innerHTML = `¿Qué país representa esta bandera?<br><img src="${pais.flags.svg}" alt="Bandera" style="width: 100px;">`;
...
function preguntaLimitrofes() {
  const pais =
    paisesConFronteras[Math.floor(Math.random() * paisesConFronteras.length)];
  ...
  document.getElementById(
    "pregunta"
  ).textContent = `¿Cuántos países limítrofes tiene ${pais.name.common}?`;
...
function mostrarOpciones(opciones, correcta, valorPuntaje = 0) {
  ...
  opciones.forEach((opcion) => {
    ...
    boton.onclick = () =>
      responder(opcion === correcta, correcta, valorPuntaje);
...
async function mostrarRanking() {
  try {
    ...
    data.rankingTiempo.forEach((item, i) => {
      const li = document.createElement("li");
      li.textContent = `#${i + 1}  ${item.nombreJugador} : ${
        item.tiempoTotal
      }s`;
...
function salirDelJuego() {
  if (
    confirm("¿Seguro que quieres salir del juego? Se perderá todo el progreso.")
  ) {
    location.reload();
  }
}
